class classes.Frame extends MovieClip
{
   ...
   function createMap()
   {
      ...
      _loc4_.onLoadInit = this.promoLoaded;
      var _loc3_ = new MovieClipLoader();
      _loc3_.addListener(_loc4_);
      _loc3_.loadClip(_global.assetPath + "/misc/promos.swf",this.map.promos);
   ...
   function promoLoaded(target_mc)
   {
      ...
      classes.Frame._MC._promosTarget.init(_loc1_);
   }
   function destroyMap()
   {
   ...
   function initTabs()
   {
      function skinHudCounter(field, accentColor)
      {
         if(field == undefined)
         {
            return undefined;
         }
         var hudFormat = field.getTextFormat();
         hudFormat.color = 16777215;
         hudFormat.bold = true;
         field.setTextFormat(hudFormat);
         field.setNewTextFormat(hudFormat);
         field.filters = [new flash.filters.GlowFilter(accentColor,0.9,5,5,2,2,false,false)];
      }
      function refreshHudCounters()
      {
         var account = _global.loginXML.firstChild.firstChild.attributes;
         classes.Frame._MC.overlay.txtMoney = "$: " + classes.NumFuncs.commaFormat(Number(account.m));
         classes.Frame._MC.overlay.txtCred = "SC: " + classes.NumFuncs.commaFormat(Number(account.sc));
         classes.Frame._MC.overlay.txtPoints = "P: " + classes.NumFuncs.commaFormat(Number(account.p));
      }
      function skinDockTab(tab)
      {
         var dockColor = new Color(tab);
         dockColor.setTransform({ra:58,ga:58,ba:58,aa:100,rb:0,gb:0,bb:0,ab:0});
         if(tab.modernAccent == undefined)
         {
            tab.createEmptyMovieClip("modernAccent",tab.getNextHighestDepth());
            classes.Drawing.rect(tab.modernAccent,tab._width - 2,2,16726051,100,1,tab._height - 3);
         }
      }
      skinHudCounter(classes.Frame._MC.overlay.fldMoney,64131);
      skinHudCounter(classes.Frame._MC.overlay.fldCred,3791848);
      skinHudCounter(classes.Frame._MC.overlay.fldPoints,16746240);
      refreshHudCounters();
      clearInterval(classes.Frame._MC.overlay.hudCounterInterval);
      classes.Frame._MC.overlay.hudCounterInterval = setInterval(refreshHudCounters,250);
      skinDockTab(classes.Frame._MC.overlay.tabNim);
      skinDockTab(classes.Frame._MC.overlay.tabEmail);
      skinDockTab(classes.Frame._MC.overlay.tabViewer);
      skinDockTab(classes.Frame._MC.overlay.tabSupport);
   ...
   function goMainSection(sectionName, locID, skipToCar)
   {
      ...
      switch(sectionName)
      {
         ...
         case "shop":
            ...
            break;
         case "dealer":
            _loc4_ = "sectionDealer";
            break;
         case "teamhq":
            _loc4_ = "sectionTeamHQ";
            break;
         case "track":
            _loc4_ = "sectionTrack";
            break;
         case "impound":
            _loc4_ = "sectionImpound";
            break;
         case "classified":
            _loc4_ = "sectionClassified";
            break;
         case "modElection":
            _loc4_ = "sectionModElection";
            break;
         case "newsRoom":
            _loc4_ = "sectionNewsRoom";
      }
      ...
      if(_loc4_)
      {
         classes.Frame._MC.sectionHolder.attachMovie(_loc4_,"sectionClip",1,{objectName:_loc4_,locationID:locID});
      }
      if(skipToCar)
      {
