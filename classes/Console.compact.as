class classes.Console
{
   ...
   function Console(target)
   {
      function refreshConverse(_subject)
      {
         ...
         classes.Drawing.insetBox(_subject.tb1,tb1W,fBoxH,20,1119253,2106921,461066,16726051);
         with(_subject)
         {
            var nnode = classes.Lookup.getBuddyNode(_subject.id);
            with(tb1)
            {
               var nObj = new Object();
               nObj.loc = "";
               nObj.s = !nnode ? 0 : nnode.attributes.s;
               nObj.ul = !nnode ? "" : nnode.attributes.ul;
               moveTo(0,20);
               beginFill(1119253);
               lineTo(tb1W,20);
               lineTo(tb1W,51);
            ...
            with(tb1)
            {
               ...
               {
                  tb1.createTextField("txt_buddyloc",tb1.getNextHighestDepth(),22,60,tb1W - 22 - 106,40);
               ...
               if(nObj.loc.length)
               {
                  txt_buddyloc.text = "I\'m in " + classes.Lookup.locationName(nObj.loc) + " race track";
               }
               else if(Number(nObj.s))
               {
                  classes.Console.showOnlineStatus(txt_buddyloc,nObj.ul);
               }
               else
               {
               ...
               txt_buddyloc._width = tb1W - 22 - 106;
               if(Number(nObj.s))
               {
                  if(!tb1.icon_jumpto)
                  {
               ...
               {
                  tb1.createTextField("txt_buddyname",tb1.getNextHighestDepth(),27,27,tb1W - 27,20);
                  ...
                  txt_buddyname._width = tb1W - 27;
                  txt_buddyname.text = classes.Lookup.buddyName(_subject.id);
               }
            }
            var cRad = 5;
            var tfw = boxW - 16;
            var tfh = boxH - inBoxH - inBoxLineWidth;
            if(_subject.tf == undefined)
            {
               _subject.createTextField("tf",_subject.getNextHighestDepth(),tb1._x,tb1._y + fBoxH + midGap,tfw,tfh);
               tf.embedFonts = true;
               ...
               {
                  _subject.scroller = new controls.ScrollBar(_subject.tf,tfh,leftM + boxW - 10,tf._y);
               }
            }
            tf._height = tfh;
            ...
            _subject.scroller.resetScroller(tfh);
            ...
            if(classes.Lookup.buddyNum(nnode.attributes.id) > -1)
            {
               disableAddBuddyBtn();
            }
            else
